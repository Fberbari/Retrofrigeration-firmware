...
void Uart_write(int c, UART_HandleTypeDef *uart)
{
	if (c>=0)
	{
		...
		__HAL_UART_ENABLE_IT(device_uart, UART_IT_TXE); // Enable UART transmission interrupt
			...
			__HAL_UART_ENABLE_IT(usb_uart, UART_IT_TXE); // Enable UART transmission interrupt
			}
	}
}
...
int IsDataAvailable(UART_HandleTypeDef *uart)
{
	...
	else if (uart == usb_uart) return (uint16_t)(UART_BUFFER_SIZE + _rx_buffer2->head - _rx_buffer2->tail) % UART_BUFFER_SIZE;
	return -1;
}
